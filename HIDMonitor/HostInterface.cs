...
namespace Ex.HidInterface
{
    public class HostInterface
    {
        ...
        NamedPipeClientStream client = new NamedPipeClientStream("HIDPipe");
        ...
        private void OnReport(HidReport report)
        {
            try // Attempt the following code...
            {
                ...
                if (report.Data.Length >= 4) // Check if length of received data is greater than or equal to 4 elements (change as needed)
                {
                    ...
                    writer.WriteLine("{0}: {1}",( (char)report.Data[0]).ToString(), ((char) report.Data[1]).ToString());
                    writer.Flush();

                    

                    Console.WriteLine(report.Data[1].ToString());
                }
